...
// 서버사이드에서 TensorFlow.js와 AI 모델 동적 로드
...
const loadAIModels = async () => {
  if (!aiModels) {
    try {
      const aiModelsJson = await import("@/json/ai_models.json");
      aiModels = aiModelsJson.default.ai_models || [];
    } catch (error) {
      console.warn("AI 모델 로드 실패:", error);
      aiModels = [];
    }
  }
  return aiModels;
...
// 모델 역직렬화 함수
const deserializeModel = async (modelJson, weightsJson, tfInstance) => {
  try {
    const model = await tfInstance.models.modelFromJSON(modelJson);
    const weightsArray = Object.values(weightsJson).map((weightGroup) =>
      Object.values(weightGroup).map((value) => Number(value))
    );

    const weights = weightsArray.map((weight, index) => {
      const shape = model.weights[index].shape;
      return tfInstance.tensor(weight, shape);
    });
...
// 예측 함수
const predict = async (model, inputData, tfInstance) => {
  try {
    if (!inputData || inputData.length === 0) {
      throw new Error("입력 데이터가 비어있습니다");
    }

    const inputTensor = tfInstance.tensor2d(inputData);
...
const processServerPredictions = async (rawData) => {
  try {
    ...
    if (!models || models.length === 0) {
      console.warn("AI 모델을 찾을 수 없습니다. 예측 없이 진행합니다.");
    ...
    const loadedModels = await Promise.all(
      models.map((model) =>
        deserializeModel(model.model, model.weights, tfInstance)
      )
    );

    // 예측 수행 (병렬 처리)
    const predictions = await Promise.all(
      loadedModels.map((model) => predict(model, processedData, tfInstance))
    );
    ...
    return rawData.map((item, index) => ({
      ...item,
      예측결과: predictionAverage[index] || 0.5,
      type: "분석",
      processedAt: new Date().toISOString(),
    }));
  } catch (error) {
...
// us, kr
const crawling = async (countryCode) => {
  try {
