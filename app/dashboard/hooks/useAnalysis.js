...
const useAnalysis = (refetchInterval = 1000 * 60 * 2) => {
  ...
  // 서버에서 예측 결과가 포함된 데이터 가져오기
  const fetchServerData = async () => {
    try {
      // predictions=true로 서버사이드 예측 처리 요청
      const response = await fetch("/api/hello?predictions=true");
  ...
  const getAnalysisData = async () => {
    ...
    try {
      ...
      // 서버에서 예측 결과가 포함된 데이터 가져오기
      const serverProcessedData = await fetchServerData();

      if (!serverProcessedData || !Array.isArray(serverProcessedData)) {
        throw new Error("유효한 데이터를 가져오지 못했습니다.");
      }

      // 웹워커로 추가 분석 수행 (dividend, cashFlow, comprehensiveAnalysis)
      return new Promise((resolve, reject) => {
        ...
        // 서버에서 처리된 데이터를 웹워커로 전송하여 추가 분석 수행
        workerRef.current.postMessage({
          type: "PROCESS_DATA",
          payload: {
            serverProcessedData: serverProcessedData,
          },
        });
      });
    } catch (error) {
